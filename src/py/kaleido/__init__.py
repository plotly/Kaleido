...
async def to_image_w_browser(spec, browser, f=None, topojson=None, mapbox_token=None, debug=None, tmp_path=None):
    if debug is None:
        debug = "KALEIDO-DEBUG" in os.environ or "KALEIDO_DEBUG" in os.environ
    def check_error(res):
        if 'error' in res:
    ...
    if response_format not in _text_formats_:
        ...
        img = str.encode(img)
    if len(browser.tabs) > 1: # always leave one tab open till we want to close
