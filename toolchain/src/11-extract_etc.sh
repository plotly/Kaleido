...
mkdir -p "$BUILD_DIR"
$NO_VERBOSE || echo "We do not clean build dir, run 10-extract.sh for that"
...
if $NO_VERBOSE; then
  unzip -qq "${MAIN_DIR}/vendor/mathjax/"*.zip -d "${BUILD_DIR}/etc/"
else
...
if [[ "$PLATFORM" == "LINUX" ]]; then
  $NO_VERBOSE || echo "Running linux specific font copies"
  mkdir -p ${BUILD_DIR}/etc/
